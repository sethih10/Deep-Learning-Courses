...
class MNIST(torch.utils.data.Dataset):
    def __init__(self, data_dir, train = True, N = None):
        ...
        if train:
            self.mnist = torchvision.datasets.MNIST(root=data_dir, train=True, download=True, transform=transform)
        else:
            self.mnist = torchvision.datasets.MNIST(root=data_dir, train=False, download=True, transform=transform)
        if N is not None:
